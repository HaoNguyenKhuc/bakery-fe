...
import {
  ...
  Badge, message, Popconfirm, Empty
} from 'antd';
import {
  SearchOutlined, ImportOutlined, ExportOutlined,
  CheckOutlined, CloseOutlined, FormOutlined, InboxOutlined, EyeOutlined,
...
const WarehouseSummaryTab: React.FC<Props> = ({ warehouse }) => {
  ...
  const { data: pendingData, isLoading: pendingLoading } = useQuery({
    queryKey: ['warehouse', 'summary', 'requests', 'PENDING_APPROVAL', warehouseId],
    queryFn: () => inventoryService.getRequests({ warehouseCode: warehouseCode!, approvalStatus: 'PENDING_APPROVAL', size: 50 }),
    enabled: !!warehouseCode,
    staleTime: 15_000,
  });

  const { data: activeData, isLoading: activeLoading } = useQuery({
    queryKey: ['warehouse', 'summary', 'requests', 'APPROVED', warehouseId],
    queryFn: () => inventoryService.getRequests({ warehouseCode: warehouseCode!, approvalStatus: 'APPROVED', size: 50 }),
    enabled: !!warehouseCode,
    staleTime: 30_000,
  });

  const { data: rejectedData, isLoading: rejectedLoading } = useQuery({
    queryKey: ['warehouse', 'summary', 'requests', 'REJECTED', warehouseId],
    queryFn: () => inventoryService.getRequests({ warehouseCode: warehouseCode!, approvalStatus: 'REJECTED', size: 50 }),
    enabled: !!warehouseCode,
    staleTime: 30_000,
  });
          ...
          onClick={() =>
            transactionService
              ...
              .then(() => {
                ...
                queryClient.invalidateQueries({ queryKey: ['warehouse', 'summary', 'requests'] });
              })
  ...
  const ActionButtons = () => (
    <Space>
      ...
      <Button
        icon={<ExportOutlined />}
        onClick={() => navigate(`/warehouse/transfer/${warehouseCode}`)}
      >
        Xuất Kho
      </Button>
      <Button
        icon={<FormOutlined />}
        onClick={() => navigate(`/warehouse/adjust/${warehouseCode}`)}
      >
        Điều Chỉnh
      </Button>
    </Space>
